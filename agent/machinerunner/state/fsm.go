// Package state implements the finite state machine (FSM) for managing Ravel machine
// lifecycles. It defines state transitions, event handling, and the valid paths a
// machine can take from creation to destruction.
//
// The state machine handles the following states:
//   - created -> preparing -> stopped -> starting -> running
//   - running -> stopping -> stopped
//   - stopped -> destroying -> destroyed
//
// Each state transition generates events that are propagated through the cluster
// via NATS, enabling distributed coordination and monitoring.
package state
...
import (
	...
	"github.com/alexisbouchez/ravel/internal/sm"
)

// maxSize defines the maximum number of recent events to keep in the event history.
const maxSize = 10

// MachineState is an alias for the machine instance state structure.
type MachineState = structs.MachineInstanceState

// newEvent creates a new machine event with the specified type, origin, status, and payload.
// All events are timestamped and associated with the machine and instance IDs.
...
// PushPreparedEvent transitions the machine to the "prepared" state after successfully
// pulling the OCI image and setting up the VM environment. The machine is now ready
// to be started.
...
// PushPrepareFailedEvent records a preparation failure (e.g., image pull failure).
// The error message is captured in the event payload for debugging.
...
// PushStartEvent initiates a machine start operation, transitioning to "starting" state.
// The isRestart parameter distinguishes between user-initiated starts and automatic restarts.
