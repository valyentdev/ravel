...
type UpdateMachineMetadataRequest struct {
	MachineResolver
	Body *api.UpdateMachineMetadataPayload
}

type UpdateMachineMetadataResponse struct {
	Body *api.Machine `json:"machine"`
}

func (e *Endpoints) updateMachineMetadata(ctx context.Context, req *UpdateMachineMetadataRequest) (*UpdateMachineMetadataResponse, error) {
	machine, err := e.ravel.UpdateMachineMetadata(ctx, req.Namespace, req.Fleet, req.MachineId, req.Body.Metadata)
	if err != nil {
		e.log("Failed to update machine metadata", err)
		return nil, err
	}

	return &UpdateMachineMetadataResponse{Body: machine}, nil
}
