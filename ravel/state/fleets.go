...
func (s *State) ListFleets(ctx context.Context, namespace string, labelFilters map[string]string) ([]api.Fleet, error) {
	fleets, err := s.db.ListFleets(ctx, namespace, labelFilters)
	if err != nil {
		return nil, err
	}
...
func (s *State) UpdateFleetMetadata(ctx context.Context, fleetID string, metadata api.Metadata) error {
	tx, err := s.db.BeginTx(ctx)
	if err != nil {
		return err
	}
	...
	err = tx.UpdateFleetMetadata(ctx, fleetID, metadata)
	if err != nil {
		return err
	}
