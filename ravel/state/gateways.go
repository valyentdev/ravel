...
func (s *State) CreateGateway(gateway api.Gateway) error {
	...
	if err = cstx.Commit(ctx); err != nil { // commit cluster-state tx after pg to avoid inconsistency
		return fmt.Errorf("failed to commit tx: %w", err)
	}
...
func (s *State) DeleteGateway(ctx context.Context, id string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to delete gateway on cluster state: %w", err)
	}

	if err = cstx.Commit(ctx); err != nil { // commit cluster-state tx first to avoid untracked gateways on failure
		return fmt.Errorf("failed to commit tx: %w", err)
	}
