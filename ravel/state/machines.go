...
// UpdateMachineMetadata updates the metadata for a machine (dual write to PostgreSQL and cluster state)
func (s *State) UpdateMachineMetadata(ctx context.Context, machineID string, metadata api.Metadata) error {
	tx, err := s.db.BeginTx(ctx)
	if err != nil {
		return err
	}
	...
	if err = tx.UpdateMachineMetadata(ctx, machineID, metadata); err != nil {
		return fmt.Errorf("failed to update machine metadata on pg: %w", err)
	}

	// Update in cluster state
	if err = s.clusterState.UpdateMachineMetadata(ctx, machineID, metadata); err != nil {
		return fmt.Errorf("failed to update machine metadata on cluster state: %w", err)
	}
